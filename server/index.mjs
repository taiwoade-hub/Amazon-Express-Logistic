...
async function handleWelcome({ supabaseAdmin, req, res }) {
  ...
  const welcomeResult = await sendWelcomeEmail({ supabaseAdmin, to: user.email, name, userId: user.id })
  if (welcomeResult?.error) {
    return json(res, 502, { error: String(welcomeResult.error?.message || welcomeResult.error || 'Failed to send email') })
  }
  ...
  if (adminEmail) {
    ...
    if (adminResult?.error) {
      return json(res, 502, { error: String(adminResult.error?.message || adminResult.error || 'Failed to send email') })
    }
  }
...
async function handleSendCreatedEmail({ supabaseAdmin, req, res }) {
  ...
  const createdResult = await sendPackageCreatedEmail({ supabaseAdmin, to: senderEmail, delivery })
  if (createdResult?.error) {
    return json(res, 502, { error: String(createdResult.error?.message || createdResult.error || 'Failed to send email') })
  }
...
async function handleAdminSendStatusEmail({ supabaseAdmin, req, res }) {
  ...
  if (type === 'approved') {
    const result = await sendPackageApprovedEmail({ supabaseAdmin, to, delivery })
    if (result?.error) {
      return json(res, 502, { error: String(result.error?.message || result.error || 'Failed to send email') })
    }
    return json(res, 200, { ok: true })
  }

  if (type === 'cancelled') {
    const result = await sendPackageCancelledEmail({ supabaseAdmin, to, delivery })
    if (result?.error) {
      return json(res, 502, { error: String(result.error?.message || result.error || 'Failed to send email') })
    }
    return json(res, 200, { ok: true })
  ...
  if (!publicApiUrl) return json(res, 400, { error: 'Missing PUBLIC_API_URL on server' })
  const receiptUrl = token
    ? `${publicApiUrl}/receipts/${encodeURIComponent(delivery?.tracking_id)}.pdf?token=${encodeURIComponent(token)}`
    : ''

  const deliveredResult = await sendPackageDeliveredEmail({ supabaseAdmin, to, delivery, receiptUrl })
  if (deliveredResult?.error) {
    return json(res, 502, { error: String(deliveredResult.error?.message || deliveredResult.error || 'Failed to send email') })
  }

  const receiptResult = await sendReceiptEmail({ supabaseAdmin, to, delivery, receiptToken: token })
  if (receiptResult?.error) {
    return json(res, 502, { error: String(receiptResult.error?.message || receiptResult.error || 'Failed to send email') })
  }
  ...
  await new Promise((resolve, reject) => {
    server.once('error', (err) => {
      if (err?.code === 'EADDRINUSE') {
        reject(new Error(`Port ${port} is already in use. Stop the other process or set PORT to a free port.`))
        return
      }
      reject(err)
    })
